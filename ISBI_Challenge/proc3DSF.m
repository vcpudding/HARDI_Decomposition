...
SNRs          = [ 10 20 30 ];
PHANTOM_names = { 'Training_3D_SF' }; % in the folder "Phantoms"
bVal          = 2000;
order         = 16;
...
for denoise = 1
    ...
    logFileID = fopen(sprintf('Results/%s__B=%04d__SNR=%02d__DELTA=auto__DENOISE=%d_GD3D.txt', PHANTOM_name{1}, bVal, SNR, denoise), 'w');
...
%         [dirs, weights, stat] = deconvolveFibersGD(S, GradientOrientations', bVal, 3, deconvOptions);
        ...
        estimateOptions.fractionThres = 1e-2;
...
%         myESTIMATION.ODF(x,y,z,:) = [ ????? ];
    ...
    % save the results
    save( sprintf('Results/%s__B=%04d__SNR=%02d__DELTA=auto__DENOISE=%d_GD3D.mat', PHANTOM_name{1}, bVal, SNR, denoise), 'myESTIMATION' );
end
end
end
