...
for denoise = [1 0]
    ...
    logFileID = fopen(sprintf('Results/%s__B=%04d__SNR=%02d__DELTA=auto__DENOISE=%d_LM.txt', PHANTOM_name{1}, bVal, SNR, denoise), 'w');
    ...
    for z = 1:n3
        ...
        deconvOptions.lambda=1e-4; % initial damping parameter value
        deconvOptions.tol=1e-6; % convergence criteria
        deconvOptions.maxiter=8000;
        deconvOptions.step = 0.5;
...
%         [dirs, weights, stat] = deconvolveFibersGD(S, GradientOrientations', bVal, 3, deconvOptions);
        ...
        estimateOptions.algorithm = 1; % 0. gradient descent; 1. LM
        estimateOptions.fraction_thres = 0.2;
        
        for it=1:10
            disp(['delta: ', num2str(deconvOptions.delta)]);
            [dirs, weights, stat] = estimateFibers(double(S), GradientOrientations', bVal, estimateOptions, deconvOptions);
            if ~isempty(weights)
                break;
            end
        ...
        disp('ground truth');
        disp(trueField{x,y,z}.R);
        disp(trueField{x,y,z}.f);
        disp('estimated');
        disp(myESTIMATION.FIELD{x,y,z}.R);
...
%         myESTIMATION.ODF(x,y,z,:) = [ ????? ];
    ...
    % save the results
    save( sprintf('Results/%s__B=%04d__SNR=%02d__DELTA=auto__DENOISE=%d_LM.mat', PHANTOM_name{1}, bVal, SNR, denoise), 'myESTIMATION' );
end
end
end
